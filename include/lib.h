...
/* 
	Trie node which contains all the information
	to determine the next hop, prefix and interface
*/
...
/*
 * Inserts the prefixes of the rtable in the trie
 * */
TrieNode *insert_prefix(TrieNode *root, struct route_table_entry rtable);

/*
 * Matches a given IP with a prefix from the trie and returns
 * all the neccessary information for the sending process
 * */
struct TrieNode *get_best_route_trie(uint32_t ip_dest);
