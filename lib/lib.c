...
TrieNode *createTrie() {
	...
	DIE(node == NULL, "trie allocation failed");
	node->bit = true;
	node->left = node->right = NULL;
	return node;
}

TrieNode *createNode(bool value) {
	TrieNode *node = (TrieNode*) malloc(sizeof(TrieNode));
	// check if malloc worked
	DIE(node == NULL, "trie node allocation failed");
	node->bit = value;
...
TrieNode *insert_prefix(TrieNode *root, struct route_table_entry rtable) {
    TrieNode *crt = root;
    
	// counter sotres the number of bits set to 1 in the mask
	uint32_t mask = rtable.mask;
	int counter = 0;
	while (mask != 0) {
		if ((mask & 1) == 1) {
			counter++;
	...
	// Insert prefixes in the tree based on the length of the mask
    for (int i = 0; counter != 0; i++) {
		counter--;
        bool bit = (rtable.prefix >> i) & 1;
        if (bit == 0) {
			// 0 bits are always on the left
            if (crt->left == NULL) {
                crt->left = createNode(bit);
            }
            crt = crt->left;
        } else {
			// 1 bits are always on the right
            if (crt->right == NULL) {
                crt->right = createNode(bit);
            }
            crt = crt->right;
        }
    }

	// the leaf contains the necessary information (next_hop & interface)
    crt->next_hop = rtable.next_hop;
    crt->interface = rtable.interface;
	return root;
}
